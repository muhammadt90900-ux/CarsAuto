...
const MOCK_USERS: User[] = Array.from({ length: 32 }, (_, i) => ({
  id: `USR${String(1000 + i).padStart(5, '0')}`,
  name: ['Ahmad Khalil','Sara Mohamed','Mohammed Hassan','Zara Ahmed','Hassan Rashid','Dina Saleh',
         'Omar Farooq','Layla Tahir','Karim Mansour','Nadia Aziz','Tariq Hussain','Lina Karimi',
         'Sami Berber','Farah Nouri','Reza Tehrani','Aisha Malik','Bilal Qureshi','Hana Jafari',
         'Yusuf Osman','Mariam Diallo','Ibrahim Kofi','Fatima Ndiaye','Khalid Barakat','Rania Haddad',
         'Wassim Benali','Sirine Chahed','Mehdi Touati','Hajar Alaoui','Younes Talbi','Nour Benzara',
         'Amira Mansouri','Samir Cherkaoui'][i],
  email: `user${i + 1}@example.com`,
  role: ROLES[i % 4],
  status: STATUSES[i % 5 === 0 ? 1 : i % 7 === 0 ? 2 : 0] as Status,
  joinedAt: `202${4 + Math.floor(i / 12)}-${String((i % 12) + 1).padStart(2, '0')}-${String((i % 28) + 1).padStart(2, '0')}`,
  lastSeen: `${Math.floor(Math.random() * 48) + 1}h ago`,
  listingsCount: Math.floor(Math.random() * 40),
  flagsCount: i % 5 === 0 ? Math.floor(Math.random() * 5) + 1 : 0,
}));
...
export default function AdminUsersPage() {
  ...
  const doAction = (userId: string, action: 'ban' | 'suspend' | 'activate') => {
  ...
  const stats = {
    ...
    flagged:   users.filter(u => u.flagsCount > 0).length,
    newToday:  4,
  };
    ...
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Flagged',       value: stats.flagged, icon: AlertTriangle,color: '#f59e0b' },
          { label: 'New Today',     value: stats.newToday,icon: UserPlus,    color: '#c9a84c' },
        ].map(s => {
          const Icon = s.icon;
          return (
            ...
            {paged.map((user, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-white/60 text-center">{user.listingsCount}</td>
