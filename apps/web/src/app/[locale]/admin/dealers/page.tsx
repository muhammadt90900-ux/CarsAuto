      ...
      <div className="flex items-center justify-between">
        ...
        {counts.PENDING > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-yellow-400/10 border border-yellow-400/20">
            ...
            <span className="text-sm font-semibold text-yellow-300">{counts.PENDING} pending review</span>
          </div>
        )}
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {dealers.map((dealer, i) => {
                const statusStyle = STATUS_STYLES[dealer.status] ?? STATUS_STYLES.PENDING;
