...
const ITEMS: ModerationItem[] = [
  ...Array.from({ length: 24 }, (_, i) => ({
    id: `MOD${String(1000 + i).padStart(5, '0')}`,
    type: (['LISTING', 'IMAGE', 'TEXT', 'PROFILE', 'LISTING', 'LISTING'] as ContentType[])[i % 6],
    reason: (['AI_FLAG', 'USER_REPORT', 'MANUAL_QUEUE', 'PRICE_ANOMALY'] as ModerationReason[])[i % 4],
    status: (['PENDING', 'PENDING', 'APPROVED', 'REJECTED'] as ModerationStatus[])[i % 4],
    title: [
      'Toyota Land Cruiser 2023 VIP Edition',
      'BMW X5 M Sport Package',
      'Suspicious low-price Toyota',
      'Lexus LX570 Full Option',
      'Fake imported car listing',
      'Mercedes G63 AMG 2023',
      'Profile with inappropriate image',
      'KIA Sportage with watermarked photos',
      'Duplicate Honda listing',
      'Nissan Patrol 2022 LE',
    ][i % 10],
    submittedBy: ['Ahmad K.', 'System AI', 'Sara M.', 'Omar F.', 'Layla T.'][i % 5],
    submittedAt: `${Math.floor(i / 3) + 1}h ago`,
    aiScore: [85, 42, 92, 18, 76, 33, 88, 55, 67, 29][i % 10],
    flags: [
      ['Price too low', 'Possible fraud'],
      ['Watermark detected'],
      ['Profanity in description'],
      ['Similar to existing listing'],
      ['Stock photo detected', 'Missing details'],
    ][i % 5],
  })),
];
...
export default function AdminModerationPage() {
  const [items, setItems]         = useState<ModerationItem[]>(ITEMS);
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Pending Review', value: pendingCount,                                             color: '#f59e0b', icon: Clock         },
          { label: 'High Risk (AI)', value: highRiskCount,                                            color: '#ef4444', icon: Zap           },
          { label: 'Approved Today', value: items.filter(i => i.status === 'APPROVED').length,        color: '#22c55e', icon: CheckCircle2  },
          { label: 'Rejected Today', value: items.filter(i => i.status === 'REJECTED').length,        color: '#8b5cf6', icon: XCircle       },
        ].map(s => {
          const Icon = s.icon;
          return (
      ...
      <div className="rounded-2xl border border-white/[0.07] overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {paged.map((item, i) => {
              ...
              const reasonStyle = REASON_STYLES[item.reason];
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3"><AiScoreBadge score={item.aiScore} /></td>
                  <td className="px-4 py-3">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-t border-white/[0.07] bg-white/[0.01]">
          <p className="text-xs text-white/30">
            {((page - 1) * PAGE_SIZE) + 1}–{Math.min(page * PAGE_SIZE, filtered.length)} of {filtered.length} items
          </p>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="p-4 rounded-xl bg-white/[0.04] border border-white/[0.07] space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <AiScoreBadge score={preview.aiScore} />
                </div>
              </div>
              <div>
