...
import { Heart, Store, Star, MapPin, Car } from 'lucide-react';
...
export default function FavoritesPage() {
  ...
  return (
    <div className="p-5 lg:p-7 space-y-5" dir={rtl ? 'rtl' : 'ltr'}>
      <div>
        <h1 className="text-xl font-bold text-primary tracking-tight">{t('favorites')}</h1>
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 dark:bg-white/[0.04] rounded-2xl p-1 w-fit">
        <button
          ...
          className={`flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-semibold transition-base
            ${activeTab === 'listings'
              ? 'bg-white dark:bg-ink-750 text-gold shadow-gold-sm'
              : 'text-secondary hover:text-primary'}`}
        >
          <Heart className="w-3.5 h-3.5" />
          {t('savedListings')}
          {savedListings.length > 0 && (
            <span className="badge badge-gold">{savedListings.length}</span>
          )}
        </button>
        <button
        ...
        >
          ...
          {followedDealers && followedDealers.length > 0 && (
            <span className="badge badge-gold">{followedDealers.length}</span>
          )}
        </button>
      </div>
      ...
      {activeTab === 'listings' && (
        favoritesLoading ? (
          ...
          <div className="text-center py-20">
            <Heart className="w-12 h-12 text-faint mx-auto mb-4" aria-hidden />
            <h3 className="font-semibold text-secondary">{t('noFavorites')}</h3>
            <p className="text-sm text-muted mt-1">{t('noFavoritesDesc')}</p>
            <Link href="/cars" className="link-gold text-xs font-semibold mt-3 inline-block">
              {t('browseListings')} →
            </Link>
          </div>
        ) : (
        ...
        dealersLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="h-40 rounded-2xl bg-gray-100 dark:bg-white/[0.04] skeleton" />
            ))}
          </div>
        ) : !followedDealers || followedDealers.length === 0 ? (
          <div className="text-center py-20">
            <Store className="w-12 h-12 text-faint mx-auto mb-4" aria-hidden />
            <h3 className="font-semibold text-secondary">{t('noFollowedDealers')}</h3>
            <p className="text-sm text-muted mt-1">{t('noFollowedDealersDesc')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {followedDealers.map(({ dealer }) => {
              ...
              return (
                <Link
                  ...
                  className="card-premium rounded-2xl p-4 transition-base group"
                >
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-primary text-sm truncate">{name}</h3>
                  ...
                  {latestListing ? (
                    <div className="flex items-center gap-2 p-2 rounded-xl bg-gray-50 dark:bg-white/[0.03] group-hover:bg-gold-50 transition-colors">
                      ...
                      <div className="min-w-0">
                        <p className="text-[10px] text-faint uppercase tracking-wide">{t('latestListing')}</p>
                        <p className="text-xs font-semibold text-secondary truncate">{listingTitle}</p>
                      </div>
                    </div>
                  ) : (
                    <p className="text-xs text-muted">{t('noActiveListings')}</p>
                  )}
                </Link>
              );
            })}
