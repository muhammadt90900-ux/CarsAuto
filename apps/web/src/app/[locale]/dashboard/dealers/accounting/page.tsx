...
export default function DealerAccountingPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl">
        ...
        {(['daily', 'weekly', 'monthly', 'yearly'] as const).map(p => (
          <button
            ...
            className={cn(
              ...
              period === p ? 'bg-gradient-to-r from-[var(--gold)] to-[var(--gold-light)] text-[#0d1b2e]' : 'text-white/40 hover:text-white/70',
            )}
          >
            {p}
          </button>
      ...
      {/* Add expense form */}
      {showAddExpense && (
        <form onSubmit={handleAddExpense} className="p-5 rounded-2xl bg-[#0d1b2e] border border-[rgba(201,168,76,0.25)] space-y-4">
