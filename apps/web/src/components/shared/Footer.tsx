...
// Redesigned — Unified gold & midnight-navy design language
'use client';
...
/* ── Social SVGs ─────────────────────────────────────────────── */
const TwitterX  = () => (<svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.253 5.622 5.91-5.622Zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>);
const Instagram = () => (<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r=".8" fill="currentColor" stroke="none"/></svg>);
const Facebook  = () => (<svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>);
const YouTube   = () => (<svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 0 0 1.46 6.42 29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58 2.78 2.78 0 0 0 1.95 1.96C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.96-1.96A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58zM9.75 15.02V8.98L15.5 12l-5.75 3.02z"/></svg>);
const WhatsApp  = () => (<svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/></svg>);
...
const PlayIcon = () => (
  <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M3.18 23.76c.3.17.65.19.97.07l11.65-6.73-2.55-2.55-10.07 9.21zm-1.93-20.4C1.1 3.64 1 3.96 1 4.31v15.38c0 .35.1.67.25.95l.08.08 8.62-8.62v-.2L1.33 3.28l-.08.08zM20.49 10.7l-2.43-1.41-2.87 2.87 2.87 2.87 2.46-1.42c.7-.4.7-1.51-.03-1.91zm-17.31 12.5l10.07-9.22-2.55-2.55L1.25 22.8l1.93 1.4z"/>
  </svg>
);
...
export function Footer({ className }: { className?: string }) {
  ...
  return (
    <footer
      ...
      style={{
        background: 'linear-gradient(175deg, #050b14 0%, #080f1c 60%, #050b14 100%)',
      }}
    >
      {/* Gold top accent */}
      <div className="absolute inset-x-0 top-0 gold-line h-[2px]" />

      {/* Ambient glow */}
      <div className="pointer-events-none absolute -top-40 left-1/2 -translate-x-1/2
                      w-[700px] h-[350px] rounded-full
                      bg-[#c9a84c]/[0.04] blur-3xl" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10 lg:gap-8">
          ...
          <div className="sm:col-span-2 lg:col-span-2 flex flex-col gap-6">
            ...
            <Link href={`/${locale}`} className="inline-flex items-center gap-2 group w-fit">
              <span className="flex items-center justify-center w-10 h-10 rounded-[11px]
                               bg-gradient-to-br from-[#c9a84c] to-[#9e6e1e]
                               shadow-[0_0_20px_rgba(201,168,76,0.35)]
                               transition-all duration-300
                               group-hover:shadow-[0_0_28px_rgba(201,168,76,0.55)]
                               group-hover:scale-[1.04] flex-shrink-0">
              ...
              <span className="text-[1.15rem] font-display font-extrabold tracking-tight leading-none">
              ...
              {socials.map(({ href, label, icon }) => (
                <a
                  ...
                  className="flex items-center justify-center w-9 h-9 rounded-xl
                             ...
                             hover:bg-[#c9a84c]/[0.10] hover:border-[#c9a84c]/30
            ...
            <div className={`flex items-center gap-2 flex-wrap ${isRTL ? 'flex-row-reverse' : ''}`}>
              {[
                ...
                { icon: <PlayIcon />,  label: 'Google Play' },
              ].map(({ icon, label }) => (
                <a key={label} href="#"
                   className="flex items-center gap-2 px-3 py-2 rounded-xl
                              bg-white/[0.05] border border-white/[0.08]
                              text-xs font-medium text-white/50
                              hover:bg-white/[0.09] hover:border-white/[0.15] hover:text-white/80
                              transition-all duration-200">
                  {icon}
                  {label}
                </a>
          ...
          {sections.map(({ title, links }) => (
            <div key={title} className="flex flex-col gap-4">
              <h4 className="text-[10px] font-bold tracking-[0.14em] uppercase text-[#c9a84c]">
                {title}
              </h4>
