...
export function DealerCard({ dealer, locale = 'en' }: DealerCardProps) {
  ...
  const ratingStars = (r: number) =>
    Array.from({ length: 5 }, (_, i) => (
      <Star
        key={i}
        className={cn('w-3 h-3', i < Math.floor(r) ? 'text-[#c9a84c] fill-[#c9a84c]' : 'text-white/15')}
      />
    ));
      ...
      <div className="relative h-28 bg-gradient-to-br from-[#0b1a2e] to-[#162840] overflow-hidden flex-shrink-0">
        ...
        <div className="absolute -bottom-5 left-4 w-12 h-12 rounded-xl border-2 border-[#0d1b2e] overflow-hidden bg-[#0d1b2e] flex items-center justify-center shadow-lg">
          {dealer.logoUrl ? (
            ...
            <span className="text-lg font-black text-[#c9a84c]">{name.charAt(0)}</span>
          )}
        </div>
      </div>
      ...
      <div className="pt-8 px-4 pb-4 flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-start gap-2">
          <div className="flex-1 min-w-0">
            <h3 className="font-display font-bold text-white text-[0.95rem] leading-snug line-clamp-1 group-hover:text-[#e8cc7a] transition-colors">
              {name}
            </h3>
            {dealer.taglineEn && (
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[0.75rem] font-bold text-[#e8cc7a]">{dealer.averageRating.toFixed(1)}</span>
        ...
        {dealer.badges && dealer.badges.length > 0 && (
          <div className="flex items-center gap-1.5">
            {dealer.badges.slice(0, 2).map(b => (
              <span key={b.code} className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#c9a84c]/10 border border-[#c9a84c]/20 text-[0.62rem] text-[#e8cc7a]">
        ...
        {dealer.whatsapp && (
          <button
            ...
            className="mt-auto flex items-center justify-center gap-2 w-full py-2 rounded-xl bg-[#25D366]/10 border border-[#25D366]/20 text-[#25D366] text-[0.75rem] font-semibold hover:bg-[#25D366]/20 transition-colors"
          >
