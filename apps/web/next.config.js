...
const nextConfig = {
  ...
  experimental: {
    ...
    // Re-enable by adding critters to devDependencies: npm install -D critters
    ...
    // NOTE: ppr (Partial Pre-rendering) and serverComponentsHmrCache require
    // Next.js canary and are not supported in stable ^14.x releases.
    // Re-enable when upgrading to Next.js 15+ canary:
    //   ppr: true,
  ...
  async headers() {
    return [
      ...
      // PWA: service worker — never cache, always fresh
      {
        source: '/sw.js',
        headers: [
      ...
      // PWA: manifest — short cache so icon/name updates propagate quickly
      {
      ...
      // PWA: offline fallback page — must be cacheable
      {
        source: '/offline',
