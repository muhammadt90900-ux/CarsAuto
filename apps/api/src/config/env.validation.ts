  ...
  if (isProd) {
    ...
    for (const key of secrets) {
      ...
      if (val && INSECURE_DEFAULTS.has(val)) {
        errors.push(`${key} contains an insecure placeholder — set a real secret in production`);
      }
    }
    ...
    if (dbUrl.includes('localhost') || dbUrl.includes('127.0.0.1')) {
      errors.push('DATABASE_URL points to localhost — use a proper production database host');
    }
    ...
    // Warn if Stripe is not set (payments will be silently disabled)
    ...
    // Warn if Cloudinary is not set (image uploads will be disabled)
    ...
    if (!hasSmtp && !hasGmail) {
      warnings.push('Email (SMTP) not configured — transactional emails will be silently skipped');
    }
  }
