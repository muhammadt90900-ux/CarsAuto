...
export class AdminService {
  ...
  async resolveReport(id: string, action: 'resolved' | 'dismissed') {
    ...
    // F-MED fix: Report.status is now the ReportStatus enum (uppercase) —
    // `action` stays the existing lowercase wire-level contract (no known
    // caller sends anything else), just mapped to the enum at the DB boundary.
